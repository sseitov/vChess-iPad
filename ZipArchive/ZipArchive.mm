...
//	zipInfo.dosDate = (unsigned long) current;
	...
	if( attr )
	{
		...
		//	zipInfo.dosDate = [fileDate timeIntervalSinceDate:[self Date1980] ];
			...
			zipInfo.tmz_date.tm_sec = (unsigned)[dc second];
			zipInfo.tmz_date.tm_min = (unsigned)[dc minute];
			zipInfo.tmz_date.tm_hour = (unsigned)[dc hour];
			zipInfo.tmz_date.tm_mday = (unsigned)[dc day];
			zipInfo.tmz_date.tm_mon = (unsigned)[dc month] - 1;
			zipInfo.tmz_date.tm_year = (unsigned)[dc year];
		}
	}
	...
	else
	{
		...
		crcValue = crc32( crcValue, (const Bytef*)[data bytes], (unsigned)[data length] );
